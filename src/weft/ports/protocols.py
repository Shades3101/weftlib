...
weft builds requests and parses responses. Somebody has to actually send
...
the right methods and should not need to be told about weft to qualify.
...
from weft.ports.dto import FetchedPage, HttpRequest, HttpResponse
...
class Transport(Protocol):
    ...
    weft deliberately cannot enforce any of it — that is the caller's
...
class Renderer(Protocol):
    ...
    expensive than a GET, so :func:`weft.extract.escalation.should_escalate`
...
class Cache(Protocol):
    ...
    at all. weft never calls it — the type exists so a caller's fetcher can
